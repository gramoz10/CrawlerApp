...
import { IMetatagModel } from "../models/metatag.model";
const requestModule = require("request");
const cheerio = require('cheerio');
var HTMLParser = require('node-html-parser');
var metatagCrawler = require('metatag-crawler');
    ...
    return new Promise((resolve, reject) => {
        ...
        requestModule(url, async (error: any, response: { statusCode: number; }, html: any) => {
            if (!error && response.statusCode == 200) {
                ...
                var crawledMetatags = await getMetatags(url);

                var metatags = crawledMetatags as IMetatagModel;
                ...
                var object: ICrawledPageModel = {
                    ...
                    "metatags": {
                        ...
                        "meta": {
                            "canonical": metatags.meta?.canonical,
                            "description": metatags.meta?.description,
...
function getMetatags(url: string) {
    return new Promise((resolve, reject) => {
        metatagCrawler(url, function (err: any, data: any) {
            resolve(data);
        });
    });
}
